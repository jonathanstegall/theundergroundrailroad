...
function writeKeywords() {
	...
	if ($subpage == 'index') {
		...
		$pagequery = "SELECT pageKeywords from tbl_mainPages WHERE pageCategory = '$catid' AND pageName = '$page'";
		$pageresult=mysql_query($pagequery);
		$pagenum=mysql_num_rows($pageresult);
		// and make pages out of them
		$i=0;
		for ($i = 0; $i < $pagenum; $i++) {
			$keywords=mysql_result($pageresult,$i,"pageKeywords");
			echo $keywords;
		}
	} else {
		...
		$subpagequery = "SELECT subPageKeywords from ".$tableName." WHERE subPageMainID = '$pageid' AND subPageName = '$subpage'";
		$subpageresult=mysql_query($subpagequery);
		$subpageum=mysql_num_rows($subpageresult);
		// and make pages out of them
		$i=0;
		for ($i = 0; $i < $subpageum; $i++) {
			$keywords=mysql_result($subpageresult,$i,"subPageKeywords");
			echo $keywords;
		}
	}
};
