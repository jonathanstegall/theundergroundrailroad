	...
	function writeDescriptions() {
		...
		if ($truepage == 1) {
			...
			for ($i = 0; $i < $pagenum; $i++) {
				...
				$pagedescription=mysql_result($pageresult,$i,"pageDescription");
			}
			if ($subpage == '' && $record == '') {
		...
		if ($truesubpage == 1) {
			...
			for ($y = 0; $y < $subpagenum; $y++) {
				...
				$subpagedescription=mysql_result($subpageresult,$y,"subPageDescription");
			}
			if (!$record) {
				echo $subpagedescription;
				return true;
			}
		} else {
			echo 'Invalid content';
		...
		if ($truerecord == 1) {
			...
			for ($n = 0; $n < $recordnum; $n++) {
				...
				$recorddescription=mysql_result($recordresult,$n,"recordDescription");
			}
